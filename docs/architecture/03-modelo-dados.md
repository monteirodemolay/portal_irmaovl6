...
query (estratégia *pool model* — ver doc 01 §1.6).
...
Auth (atualizados por Cloud Function quando o papel do usuário muda — ver doc
07). Isso evita uma leitura extra ao Firestore a cada avaliação de regra.

`auditLogs` tem regra especial: **write somente via Cloud Function
(Admin SDK)**, nunca diretamente do client — garante integridade da trilha.

## 3.5 Exportação / Backup

- Backup automático diário via Cloud Function agendada (`scheduled/daily-backup-export.ts`)
  usando `firestore.export` para um bucket dedicado (retenção 90 dias).
