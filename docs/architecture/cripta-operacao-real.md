...
O formulário mantém uma **prévia na memória da aba**; fechar a aba apaga a prévia. O botão de ensaio conectado é separado e restrito à conta piloto: aceita somente carta fictícia e até 650 KB de anexos no total, cifra no navegador, deposita o pacote privado no Wix, registra índice opaco no Firestore e permite baixar, decifrar e solicitar exclusão. O acesso ao ensaio expira em 48 horas; uma rotina diária tenta remover objetos expirados e falhas precisam de reconciliação. Esse fluxo não é a operação anual e não contém recuperação de chaves, mídias grandes ou exportação física. A tela operacional só poderá aceitar conteúdo real depois de testar todos os passos de cifra, depósito, restauração e exportação para três unidades externas.
