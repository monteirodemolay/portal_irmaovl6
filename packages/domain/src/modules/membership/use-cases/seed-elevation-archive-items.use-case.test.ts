...
import { CreateElevationArchiveItemUseCase } from '../../archive/use-cases/create-elevation-archive-item.use-case';
import type { Member } from '../entities/member.entity';
import { SeedElevationArchiveItemsUseCase } from './seed-elevation-archive-items.use-case';
...
function buildUseCase(members: Member[]) {
  ...
  const useCase = new SeedElevationArchiveItemsUseCase({
    memberRepository,
...
describe('SeedElevationArchiveItemsUseCase', () => {
  it('cria item de elevação só pros Irmãos com dataElevacao que ainda não têm um', async () => {
    const semItem1 = buildMember({
      id: 'm2',
      nomeCompleto: 'Maria Pendente',
      dataElevacao: new Date('2015-05-20T00:00:00Z'),
    });
    const semItem2 = buildMember({
      id: 'm3',
      nomeCompleto: 'Pedro Pendente',
      dataElevacao: new Date('2018-11-02T00:00:00Z'),
    });
    const semDataElevacao = buildMember({
      id: 'm4',
      nomeCompleto: 'Sem Data',
      dataElevacao: null,
    });

    const { useCase, archiveItemRepository } = buildUseCase([semItem1, semItem2, semDataElevacao]);
