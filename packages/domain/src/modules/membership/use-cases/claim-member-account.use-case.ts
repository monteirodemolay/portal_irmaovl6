...
/**
 * Único Use Case do domínio sem `AuthContext` — desvio deliberado do
 ...
 * criação da conta Firebase Auth/`User` (com a senha que o próprio Irmão
 * escolheu) continua na Server Action (`claimMemberAccountAction`), mesma
 * divisão de responsabilidade que `createPortalAccessForMember` já usa
 * (infraestrutura fora do domínio). Libera o acesso na hora — sem fila de
 * aprovação do Administrador, de propósito: o par Nome+CIM (reforçado por
 * rate limit por IP na Action) já é a prova de identidade suficiente pra
 * este fluxo público autoatendido (docs/architecture/07 §7.2b).
 */
export class ClaimMemberAccountUseCase {
  ...
  async execute(
    ...
    // Mesma regra de `RegisterMemberSituationUseCase`: nenhum acesso ao
    // Portal fora da situação "Ativo" (licenciado/suspenso/desligado/In
    // Memoriam nunca reivindicam acesso por aqui).
    if (member.situacao !== 'ativo') {
