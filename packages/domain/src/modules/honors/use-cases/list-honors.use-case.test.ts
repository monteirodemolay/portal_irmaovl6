...
import { ListHonorsUseCase } from './list-honors.use-case';
...
describe('ListHonorsUseCase', () => {
  it('lista só as honrarias do Irmão e do tenant, mais recente primeiro', async () => {
    ...
    await honorRepository.create(
      buildHonor({
        id: 'honor-2',
        nomeOficial: 'Medalha da Fraternidade',
        data: new Date('2020-08-12'),
      }),
    );
    await honorRepository.create(buildHonor({ id: 'honor-3', memberId: 'member-2' }));

    const useCase = new ListHonorsUseCase({ honorRepository });
