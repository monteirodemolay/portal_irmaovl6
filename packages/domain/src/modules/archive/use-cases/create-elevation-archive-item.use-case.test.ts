...
import { CreateElevationArchiveItemUseCase } from './create-elevation-archive-item.use-case';
...
function buildUseCase() {
  ...
  const useCase = new CreateElevationArchiveItemUseCase({
...
describe('CreateElevationArchiveItemUseCase', () => {
  ...
  it('cria um Evento mínimo de grau Companheiro quando não há nenhum na data', async () => {
    ...
    const result = await useCase.execute(ctx, {
      ...
      dataElevacao: new Date('2025-07-20T12:00:00Z'),
    });
    ...
    expect(events[0]?.grau).toBe('companheiro');
  ...
  it('usa o endereço cadastrado da Loja como local do Evento criado, quando existe', async () => {
    ...
    await useCase.execute(ctx, {
      ...
      dataElevacao: new Date('2025-08-10T12:00:00Z'),
    });
  ...
  it('Irmão com iniciação E elevação ganha dois ArchiveItems distintos, um por marco', async () => {
