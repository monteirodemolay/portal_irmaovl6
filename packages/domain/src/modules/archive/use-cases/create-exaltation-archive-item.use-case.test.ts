...
describe('CreateExaltationArchiveItemUseCase', () => {
  it('reaproveita um Evento já existente na mesma data, sem criar um novo', async () => {
    ...
    expect(result.archiveItem.titulo).toBe('Exaltação — 15/06/2025');
    expect(result.archiveItem.publicacaoStatus).toBe('rascunho');
    expect(result.archiveItem.origemExaltacaoMemberIds).toEqual(['member-1']);
  });
  ...
  it('Irmãos exaltados juntos na mesma data compartilham um único ArchiveItem', async () => {
    ...
    const second = await useCase.execute(ctx, {
      memberId: 'member-2',
      nomeCompleto: 'Maria Souza',
      dataExaltacao: new Date('2025-06-15T09:00:00Z'),
    });

    expect(second.memberAdded).toBe(true);
    expect(second.archiveItem.id).toBe(first.archiveItem.id);
    expect(second.archiveItem.origemExaltacaoMemberIds).toEqual(['member-1', 'member-2']);
