...
import { ListParamasonicEntityMembersUseCase } from './list-paramasonic-entity-members.use-case';
...
async function buildScenario() {
  ...
  const useCase = new ListParamasonicEntityMembersUseCase({
    paramasonicEntityMemberRepository,
    memberRepository,
  });
...
describe('ListParamasonicEntityMembersUseCase', () => {
  it('lista os integrantes resolvendo o nome do Irmão vinculado', async () => {
    const { useCase, entityId } = await buildScenario();

    const result = await useCase.execute(ctx, entityId);

    expect(result.map((m) => m.nomeCompleto)).toEqual(['Irmão Titular', 'Zeca Souza']);
    const irmao = result.find((m) => m.memberId === 'member-1');
    expect(irmao?.cargo).toBe('Conselheiro');
  });
